...
class DocumentIndexEntry(Document):
    ...
    # Tracks the background enrichment phase, which is entirely separate from
    # Backboard's indexing `status` above (a doc reads "indexed" the whole time
    # enrichment is still running). "none" for docs that aren't repo-scoped and
    # so never enrich. See app/file_upload/enrichment.py.
    ...
    # decision memories the doc produced.
